...
func TestSwarmScalability(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			size:               100,
			expectedBatching:   false,
			expectedWorkerPool: true, // AutoScaleConfig does set MaxConcurrentAgents for 100
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			config := AutoScaleConfig(tt.size)
			swarm, err := NewSwarm(tt.size, State{
				Phase:     0,
