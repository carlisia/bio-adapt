...
func main() {
	...
	config := attractor.ConfigForBatching(numWorkloads, batchWindow)
	swarm, err := attractor.NewSwarm(numWorkloads, targetState, attractor.WithConfig(config))
	if err != nil {
...
results:
	...
	fmt.Println("batchConfig := attractor.State{")
	...
	fmt.Println("workloads, _ := attractor.NewSwarm(50, batchConfig)")
	fmt.Println()
	...
	fmt.Println("├─ The attractor package provides the coordination logic")
...
func visualizeRequestTimeline(swarm *attractor.Swarm) {
