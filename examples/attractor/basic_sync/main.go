...
func main() {
	...
	config := attractor.AutoScaleConfig(swarmSize)
	swarm, err := attractor.NewSwarm(swarmSize, targetState, attractor.WithConfig(config))
	if err != nil {
	...
	fmt.Println("targetState := attractor.State{")
	...
	fmt.Println("swarm, err := attractor.NewSwarm(10, targetState)")
	fmt.Println()
...
func visualizeAgents(swarm *attractor.Swarm) {
